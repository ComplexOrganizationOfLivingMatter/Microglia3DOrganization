function tabla = prepareIndividualNonPlaqueSphereDataTable(tabla, nImages)
%PREPAREINDIVIDUALNONPLAQUESPHEREDATATABLE Initialize the result table for individual AD non-plaque spheres.
%
...
% MATLAB image order unless the argument name explicitly states XYZ.
    ...
    tabla.PlaqueInfluenceIndex_1 = nan(nImages,1);
    tabla.PlaqueInfluenceIndex_2 = nan(nImages,1);
    tabla.PlaqueInfluenceIndex_3 = nan(nImages,1);
    tabla.MaxInfluencePlaque = nan(nImages,1);
    
    tabla.plaqueDirDispersion = nan(nImages,1);
    tabla.plaqueDirDispersion_weighted = nan(nImages,1);
    tabla.u_influence_x = nan(nImages,1);
    tabla.u_influence_y = nan(nImages,1);
    tabla.u_influence_z = nan(nImages,1);
    tabla.AngleUInfluenceMaxAlign = nan(nImages,1);
    tabla.AngleUInfluenceMaxCos = nan(nImages,1);
