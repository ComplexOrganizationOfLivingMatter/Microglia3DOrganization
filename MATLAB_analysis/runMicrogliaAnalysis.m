...
%       Opens the interactive configuration workflow.
%
%   runMicrogliaAnalysis(configStruct)
%       Executes the pipeline with an existing configuration structure.
%
%   runMicrogliaAnalysis(configFile)
%       Loads a JSON or MAT configuration file and executes the pipeline.
    ...
    metadataCleanup = onCleanup(@() removeTemporaryMetadata(metadataFile, config));
    derivedCleanup = onCleanup(@() cleanupTemporaryDerivedImages(config));
