...
%   getVoronoiLayerData
    ...
    if useParallel
        ...
        workerFile = which('processCorticalLayerImage');
        if isempty(workerFile)
            error('The cortical-layer worker file processCorticalLayerImage.m was not found on the MATLAB path.');
        end
        ...
        parfor ii = 1:numel(filesToProcess)
            ...
            results{ii} = processCorticalLayerImage(task);
        end
    else
