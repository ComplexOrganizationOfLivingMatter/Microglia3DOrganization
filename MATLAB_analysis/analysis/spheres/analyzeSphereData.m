...
%      consistently with plaque spheres.
    ...
    isADImage = arrayfun(@(x) isADModelLabel(x), string(tableInfo.Modelo));
    ...
    if nargin < 11 || isempty(radiusPairs_um), radiusPairs_um = [100 100]; end
    ...
    cleanupFigureVisibility = onCleanup(@() set(groot, 'DefaultFigureVisible', oldFigureVisible));
    ...
    for r = 1:size(radiusPairs_um,1)
        ...
        radiusFolderName = getSphereRadiusFolderName(radiusPlaqueCurrent, radiusCurrent);
        ...
        plaqueSpheresIndPath = fullfile(sphereDataInputPathR, 'SphereInd', 'PlaqueSpheresInd');
        nonPlaqueSpheresIndPath = fullfile(sphereDataInputPathR, 'SphereInd', 'NonPlaqueSpheresInd');
        wtSpheresIndPath = fullfile(sphereDataInputPathR, 'SphereInd', 'WTSpheresInd');
        ...
        for n_file = 1:nImages
            ...
            sphereImageFile = fullfile(sphereImageInputPathR, outputModel, filename);
            if isfile(sphereImageFile)
                sphereImg = readStackTif(sphereImageFile);
            else
                sphereImg = [];
            end
            ...
            if isempty(sphereImg)
                sphereImg = buildSphereMaskFromIndices([numRows, numCols, numSlices], ...
                    plaqueSpheresInd, noPlaqueSpheresInd, isADModelLabel(typeModel));
            else
            ...
            closeFiguresSafely();
                ...
                try
                    ...
                    fid = fopen(logFile, 'a');
                    fprintf(fid, '\n[%s] Radius %d | image %d/%d\n', datetime("now"), radiusCurrent, n_file, nImages);
                    if exist('name', 'var')
                        fprintf(fid, 'File: %s\n', name);
                    end
                    fprintf(fid, 'Error: %s\n', ME.message);
...
function [tableData, nSpheresCompleted] = getPlaqueSphereData_optimized_corrected( ...
    ...
    nSpheresCompleted = 0;
    nPlaquesCompletedCapt = 0; 
    nSpheresTouchPlaques = 0; 
    ...
    for p = 1:length(spheresInd)
        ...
        if diamPlaques(p) / 2 < radius
            isPlaqueInside = 1;
            nPlaquesCompletedCapt = nPlaquesCompletedCapt + 1; 
        end
        ...
        if length(unique(plaqueImage(s))) > 2
            ...
            nSpheresTouchPlaques = nSpheresTouchPlaques + 1; 
            distNearestPlaque = 0;
        ...
        if isADModelLabel(imageData{6})
            ...
            if ~isempty(centroidsPlaques)
                ...
                for nPlaque = 1:size(centroidsPlaques, 1)
                    ...
                    tableSphPlaData.DistanceToPlaqueSurface(row_plaque) = double(min(pdist2(centroidSphere, plaquePoints), [], 'all', 'linear') * xyResolution * correctionFactor);
...
function tf = isADModelLabel(value)
%ISADMODELLABEL Return true for configured AD labels used in this pipeline.
    label = upper(strtrim(string(value)));
    tf = label == "AD" || contains(label, "APP") || contains(label, "ALZHEIMER");
end

function sphereMask = buildSphereMaskFromIndices(imageSize, plaqueSpheresInd, nonPlaqueSpheresInd, isAD)
%BUILDSPHEREMASKFROMINDICES Reconstruct the analysis mask when TIFF output is disabled.
    sphereMask = zeros(imageSize, 'uint16');
    if isAD
        for i = 1:numel(plaqueSpheresInd)
            ind = double(plaqueSpheresInd{i});
            ind = ind(ind >= 1 & ind <= numel(sphereMask));
            if ~isempty(ind)
                current = uint32(sphereMask(ind)) + 1;
                sphereMask(ind) = uint16(min(current, intmax('uint16')));
            end
        end
        nonPlaqueLabel = uint16(double(max(sphereMask, [], 'all')) + 1);
        for i = 1:numel(nonPlaqueSpheresInd)
            ind = double(nonPlaqueSpheresInd{i});
            ind = ind(ind >= 1 & ind <= numel(sphereMask));
            sphereMask(ind) = nonPlaqueLabel;
        end
    else
        for i = 1:numel(nonPlaqueSpheresInd)
            ind = double(nonPlaqueSpheresInd{i});
